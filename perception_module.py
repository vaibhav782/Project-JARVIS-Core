...
# # below code has minor issues as it gets confused when asked somethings 
# import requests

# def get_weather(location: str) -> str:
#     """Fetches current weather for a given location string using wttr.in."""
#     try:
#         # Format: wttr.in/{city}?format=3 -> returns "City: +TempC, Condition"
#         url = f"https://wttr.in/{location}?format=3"
#         response = requests.get(url)
#         response.raise_for_status()
        
#         # wttr.in returns plain text for format=3
...
def get_weather(location: str) -> str:
    ...
    try:
        url = f"https://wttr.in/{location}?format=3"
        # wttr.in requires a User-Agent header or it might block the request
        headers = {
